...
public class PackageAdapter extends RecyclerView.Adapter<PackageAdapter.MyViewHolder> {
    ...
    public PackageAdapter(Context c, ArrayList<Paket> packages){
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new PackageAdapter.MyViewHolder(LayoutInflater.from(context).inflate(R.layout.custom_list_paket_pencucian , parent , false));
    }
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, final int position) {
        ...
        holder.button_pilih.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent i = new Intent();
                i.putExtra("image_package", (Serializable) packages.get(position).getPackage_photo());
                i.putExtra("price_package", packages.get(position).getPackage_price());
                i.putExtra("type_package", packages.get(position).getPackage_vehicle_type());
                i.putExtra("name_package", packages.get(position).getPackage_name());
                i.putExtra("information_package", packages.get(position).getPackage_information());
                i.putExtra("id", packages.get(position).getId());
